...
const Firestore = require('./firebaseClient');
...
useFirestoreAuthState = async (collectionName) => {

    const writeData = async (data, id) => {
        const credData=JSON.parse(JSON.stringify(data, BufferJSON.replacer))
        firebase.addDataById(collectionName,id,credData);
    }
    const readData = async (id) => {
        try {
            const data = await firebase.readDataById(collectionName, id);
    ...
    const removeData = async (id) => {
        try {
            await firebase.deleteDataById(collectionName, id);
        } catch (_a) {}
    }
